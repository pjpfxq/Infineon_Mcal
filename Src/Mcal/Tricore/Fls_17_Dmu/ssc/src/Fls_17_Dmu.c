...
#if (FLS_17_DMU_SW_MINOR_VERSION != 40U)
...

  {
    ...
    if(Fls_lChkOperError() == 0U)
    {
      ...
      if (StatePtr->FlsTimeoutControl != 0U)
      {
        ...
        StatePtr->FlsEraseTimeoutCycleCount++;
        /* Update the time out count in proportion to the
...
static uint8 Fls_lTimeOutDET(const uint8 ServiceID)
{
  ...
  if (StatePtr->FlsTimeoutControl != 0U)
  {
    ...
    if(ErrorFlag == 1U)
    {
      ...
      /* Update the global status variables */
      /* [cover parentID={6378D004-FBAA-4d94-AF15-689BDD07B20D}]
      [/cover] */
      StatePtr->FlsJobResult = MEMIF_JOB_FAILED;

      /*Error notification*/
      /* [cover parentID={72B83E93-547F-4407-9525-5E6FA811995F}]
       Is the job error notification configured
      [/cover] */
      if(Fls_ConfigPtr->FlsJobErrorNotificationPtr != NULL_PTR)
      {
        /* [cover parentID={9766AC8E-2919-421f-BE98-E3BE290387CB}]
        [/cover] */
      ...
      /* [cover parentID={9B8886B4-865E-4f02-B85F-C5526AAE82F3}]
       [/cover] */
      StatePtr->FlsJobType = FLS_17_DMU_NONE;
...
LOCAL_INLINE void Fls_lCalculateWriteTimeoutCount(void)
{
  ...
  if(Length > FLS_17_DMU_PAGE_SIZE)
  {
    ...
    /* [cover parentID={2956661C-50C9-465d-A666-61A66DCAAC0F}]
       [/cover] */
  ...
  /* Update the time out count in proportion to the number of
  write command cycles */
  /* [cover parentID={987C1E59-17F8-4033-B14E-650F67ADE80A}]
     [/cover] */
  #if(FLS_17_DMU_USE_INTERRUPTS == STD_OFF)
