...
#if ( OCU_AR_RELEASE_MAJOR_VERSION != 4U )
...
#endif
#if ( OCU_SW_MAJOR_VERSION != 10U )
#error "OCU_SW_MAJOR_VERSION does not match. "
#endif

#if ( OCU_SW_MINOR_VERSION != 40U )
                       ...
                       const Ocu_PinActionType PinAction)
{
  ...
  [/cover] */
  /* MISRA2012_RULE_14_3_JUSTIFICATION: According to AUTOSAR,
   * Input parameter "PinAction" variable is of enum data type
   * and expected to report DET/Safety Error if the Input parameter
   * "PinAction" is Invalid. No side effects foreseen by violating
   * this MISRA rule */
                      ...
                      const Ocu_PinStateType PinState)
{
  ...
  [/cover] */
  /* MISRA2012_RULE_14_3_JUSTIFICATION: According to AUTOSAR,
   * Input parameter "PinState" variable is of enum data type
   * and expected to report DET/Safety Error if the Input parameter
   * "PinState" is Invalid. No side effects foreseen by violating
   * this MISRA rule */
    ...
    #endif
    {
      ...
      switch(LocalOcuChanPtr->OcuProperties.AssignedHwUnit)
      {
        case OCU_GTM_ATOM_SHARED:
          ...
          /* MISRA2012_RULE_11_4_JUSTIFICATION: SFR access. No side effects
           * foreseenby violating this MISRA rule. */
          /* MISRA2012_RULE_11_5_JUSTIFICATION: SFR access. No side effects
           * foreseenby violating this MISRA rule. */
    ...
    const Ocu_ValueType ReferenceValue, const Ocu_ValueType AbsoluteValue )
{
  ...
  ReturnValue = OCU_CM_OUT_REF_INTERVAL;
      ...
      Channel = Channel + 1U)
  {
    ...
    {   
      ...
      else
      {
        ...
        /* [cover parentID={6063D765-6A18-4fac-B2A8-9B1E7F8E5E5D}]
        Initialize ATOM channel
        [/cover] */
        /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen
          by violating this MISRA rule. */
...
**                                                                            **
*******************************************************************************/
void Ocu_Timer_Isr (const Ocu_ChannelType Channel, uint32 flags)
/*MISRA2012_RULE_8_3_JUSTIFICATION:parameter list differs from prior
  declaration,justified*/
{
  const Ocu_ChannelConfigType *LocalOcuChanPtr; /* ROM pointer */
  uint8 CoreId;
  #if (OCU_SAFETY_ENABLE == STD_ON)
