...
#define GTM_TOM_CH_CTRL_CLK_SRC_SR_VALUE (0x07)
#define GTM_ATOM_CH_CTRL_CLK_SRC_SR_LEN   (0x04)
#define GTM_ATOM_CH_CTRL_CLK_SRC_SR_VALUE (0xC)
#define GTM_ATOM_CH_CTRL_MODE_VALUE  (0x2)
#define GTM_TOM_CH_CTRL_CLK_SRC_SR_LEN (0x3)
#define GTM_ATOM_CH_CTRL_MODE_LEN (0x2)
#endif
...
void Pwm_17_GtmCcu6_DeInit(void)
{
        ...
        (PWM_17_GTMCCU6_DEV_ERROR_DETECT == STD_ON))*/
  {
    ...
    for (;CoreTemp < PWM_17_GTMCCU6_MAX_CORES;
         CoreTemp++
        )
    #endif
    {
...
)
{
  ...
  /* Switching off the clock input to avoid the output level changes*/ 
  Mcal_SetBitAtomic  ((&TomChannelRegPtr->CTRL.U), IFX_GTM_TOM_CH_CTRL_CLK_SRC_SR_OFF, \
  GTM_TOM_CH_CTRL_CLK_SRC_SR_LEN, GTM_TOM_CH_CTRL_CLK_SRC_SR_VALUE);
...
)
{
  ...
  /* Switching off the clock input to avoid the output level changes*/ 
  Mcal_SetBitAtomic((&AtomChannelRegPtr->CTRL.U), IFX_GTM_ATOM_CH_CTRL_CLK_SRC_SR_OFF, \
  GTM_ATOM_CH_CTRL_CLK_SRC_SR_LEN, GTM_ATOM_CH_CTRL_CLK_SRC_SR_VALUE);
  
  Mcal_SetBitAtomic((&AtomChannelRegPtr->CTRL.U), IFX_GTM_ATOM_CH_CTRL_MODE_OFF,\
  GTM_ATOM_CH_CTRL_MODE_LEN, GTM_ATOM_CH_CTRL_MODE_VALUE);
  ...
  Enable update enable for the channel [/cover] */
  ResetType   = Pwm_lChanResetType(PwmChannelCfgPtr->PwmChannelInfo);
  ...
  /* [cover parentID={F6955709-4746-43fb-8435-DB3666AC8B67}]
  ...
  /* Call Mcu_17_Gtm_AtomChannelInit Library to Initialize TOM channel */
  /* MISRA2012_RULE_11_5_JUSTIFICATION: PwmTimerPtr is of
  type Mcu_17_Ccu6_TimerConfigType. No side effects foreseen
  by violating this MISRA rule.*/
  /* MISRA2012_RULE_11_3_JUSTIFICATION: PwmTimerPtr is of
...
)
{
  ...
  if(ErrorStatus == E_OK)
  {
    ...
    if(ErrorStatus == E_OK)
    {
      /* Update enable for the channel must be enabled if coherent
      update is desired */
      ResetType   = Pwm_lChanResetType(PwmChannelCfgPtr->PwmChannelInfo);
