    ...
    for(HwMap = 0; HwMap < SPI_MAX_HW_UNIT; HwMap++)
    {
      ...
      if(Spi_lIsQSPIHwConfigured(HwMap) == (uint8)E_OK)
      {
        ...
        SPI_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG(                  \
            ...
            CoreConfigPtr->QSPIHwConfigPtr[HwMap]->ClockSetting);
        #endif
...
Spi_StatusType Spi_GetHWUnitStatus(const Spi_HWUnitType HWUnit)
{
  ...
  #endif
  {
    ...
    if(HWUnit < (uint8)SPI_MAX_HW_UNIT)
    {
      ...
      {
        #if (SPI_LEVEL_DELIVERED != 1U)
        /* [cover parentID={3946A2BB-BAE2-4710-925D-AB9D1AD4B27D}][/cover] */
        if (Spi_lIsQSPIHwConfiguredSync(HWUnit) == E_OK)
        {
          /* [cover parentID={1344D9A4-5926-4cde-9568-478DA2C3E7F6}][/cover] */
        ...
        #endif
        #if (SPI_LEVEL_DELIVERED != 0U)
        /* [cover parentID={B806B93D-9B80-4d14-9A0F-9D0A4DA011E9}][/cover] */
        if (Spi_lIsQSPIHwConfiguredAsync(HWUnit) == E_OK)
        {
          /* [cover parentID={71FB0256-85F8-4D34-92A5-18910364C145}][/cover] */
          if (Spi_RuntimeKernelVar[HWUnit]->BusStatus == SPI_IDLE)
          {
            HwIdle = 1U;
          }
        }
                                         ...
                                         const Spi_LoopBackType EnableOrDisable)
{
  ...
  #endif
  {
                 ...
                 (EnableOrDisable == SPI_LOOPBACK_DISABLE))
    {
      ...
      {
        #if (SPI_LEVEL_DELIVERED != 1U)
        /* [cover parentID={19DADE27-7F57-41f3-B77B-DCDC33213409}][/cover] */
        if (Spi_lIsQSPIHwConfiguredSync(HWUnit) == E_OK)
        {
          if((Spi_RuntimeCoreVar[CoreId]->KernelLock & \
                               (1UL << (uint32)HWUnit)) == 0U)
        ...
        #endif
        #if (SPI_LEVEL_DELIVERED != 0U)
        /* [cover parentID={A55CE23F-4653-4776-9289-6E1EF9C0B869}][/cover] */
        if (Spi_lIsQSPIHwConfiguredAsync(HWUnit) == E_OK)
        {
          if (Spi_RuntimeKernelVar[HWUnit]->BusStatus == SPI_IDLE)
          {
        ...
        #endif
        /* [cover parentID={D840BBAC-8F1E-425E-9E0F-22A9990B9251}][/cover] */
        if (HwIdle == 1U)
        {
           /* MISRA2012_RULE_11_5_JUSTIFICATION: Agreed violation
          ...
          /* MISRA2012_RULE_11_3_JUSTIFICATION: QSPI base address
          access */
          /* MISRA2012_RULE_11_8_JUSTIFICATION: SFR access. Since
          the input argument for the API discards volatile keyword.
          No side effects foreseen by violating this MISRA rule */
...
*******************************************************************************/
void Spi_QspiDmaErrCallout(const uint32 Channel, const Dma_EventsType Event)
{
  uint8 ModIndex;
...
static void Spi_lAsyncStartJob(const Spi_JobType Job)
{
  ...
  Dma_ChStartTransfer((uint8)(CoreConfigPtr->QSPIHwConfigPtr[HwUnit]->  \
                              DMATxChannel));
...
static void Spi_lUpdateQspiIntr(const uint8 Enable)
{
  ...
  for(Modid = 0U; Modid < SPI_MAX_HW_UNIT; Modid++)
  {
    ...
    if(Spi_lIsQSPIHwConfiguredAsync(Modid) == (uint8)E_OK)
    {
      ...
      * argument for the API discards volatile keyword. No side effects foreseen
      * by violating this MISRA rule */
      SPI_SFR_RUN_TIME_WRITE32(&SPI_HW_MODULE[Modid]. \
                                            GLOBALCON1.U, GlobalCon1Reg.U);
    }
  }
